#######################################
# Help function for single A simulation
#######################################
...
MonoSBM_single <- function(seed, K, Bmat, sparseRho, numNode, numGraph, block.sizes, 
                           lambdas, nfold=5, parallel=TRUE, ncores=5) {
  set.seed(seed)
  ...
  Winit <- matrix(0,K,K)
  cvRes <- cvSRL(seed, Alist[[1]], Z, lambdas, block.sizes, nfold, parallel, ncores)
  ...
  return(list(ourB=estB, avgB=Bhat,
              ourRank=ourRank, avgRank=avgRank,
              ourMSE=ourMSE, avgMSE=avgMSE))
}

cvSRL <- function(seed, A, Z, lambdas, block.sizes, nfold, parallel, nCores) {
  K <- ncol(Z)
  Winit <- matrix(0,K,K)
  ...
  if (parallel == TRUE) {
    ...
    cvRes <- foreach(i = 1:nfold) %dopar% {
      cvFunc(fold, i, A, Z, lambdas, Winit)
    }
    stopCluster(cl)
  } else {
    cvRes <- list()
    for (i in 1:nfold) {
      cvRes[[i]] <- cvFunc(fold, i, A, Z, lambdas, Winit)
    }
  }
